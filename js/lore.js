 ...
 *
 * 実装状況（2026-08-28 M6.6 WP4時点）：
 *   opening … WP1で実装（従来 js/run-ui.js に直書きしていたものをここへ移した）
 *   areas   … WP4で実装（開始マスの案内）
 *   hints   … WP4では carryOut のみ。残り15種は M6.5c の仕事
 *   home / nodes / result / journal … 未実装（M6.5c〜M7）
 *
...
(function (global) {
  ...
  const LORE = {
    ...
    /* 開始マスの案内（M6.6 WP4）。出す順は run-ui.js の startGuideScript()：
     *   first（初回3つ）または repeat（2回目以降・候補群から1つ）
     *   → masterIntro（初回のみ）→ fog（霧の日）／fieldRule（戦場ルールの日）
     * fieldRule は全エリア共通なので areas の外（下の common）に置いてある。 */
    areas: {
      grassland: {
        first: [                                                    /* 台本§3.1 */
          { face: 'calm', lines: ['草原だ。ここの魂は薄い。', '長く留まりすぎて、形を忘れかけている。'] },
          { face: 'calm', lines: ['道は二つに分かれる。', '強い相手ほど、良い記録になる。'] },
